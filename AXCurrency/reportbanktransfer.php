...
<table border="1" cellspacing="0" cellpadding="0" width="580px;" style="font-size:11px; font-family:arial;">
			...
			<?php
			include('db.php');
				$resulta = mysqli_query($bd,"SELECT * FROM bank_transfer WHERE bank_name='$bname' AND date BETWEEN '$a' AND '$b' order by date asc");
				...
				<td>
				<?php
				$results = mysqli_query($bd,"SELECT sum(debit) FROM bank_transfer WHERE bank_name='$bname' AND date BETWEEN '$a' AND '$b'");
				...
				<td>
				<?php
				$results = mysqli_query($bd,"SELECT sum(credit) FROM bank_transfer WHERE bank_name='$bname' AND date BETWEEN '$a' AND '$b'");
				while($row2 = mysqli_fetch_array($results))
				  {

...
$payment=$_GET['type'];
$civu=$_GET['cvcvc'];
$result = mysqli_query($bd,"SELECT * FROM report WHERE transaction_nu='$tnu'");
while($row = mysqli_fetch_array($result))
  {
  $ftd=$row['transaction_nu'];
  }
  if($ftd!=$tnu){
  mysqli_query($bd,"INSERT INTO report (transaction_nu, date, name, currency, rate, pesoval, branch, amount, ttype, payment)
VALUES ('$tnu','$da','$cname','$cur','$rate','$pesoval','$branch','$amount','$ttype','$payment')");
  if($cur=='USD'){
		if($ttype=='Buy'){
		$rem = '0';
		mysqli_query($bd,"UPDATE bank_account SET debit=debit+$amount, credit=credit, balance=balance+'$amount' WHERE bank_code='$civu'");
...
VALUES ('$tnu','$da','$amount','0','$civu','$rem','$pesoval')");
		...
		if($ttype=='Sell'){
		$rem = '0';
		mysqli_query($bd,"UPDATE bank_account SET debit=debit, credit=credit+$amount, balance=balance-'$amount' WHERE bank_code='$civu'");
		mysqli_query($bd,"INSERT INTO bank_transfer (trn_num, date, debit, credit, bank_name, remarks, pesoval) 
VALUES ('$tnu','$da','0','$amount','$civu','$rem','$pesoval')");
  ...
  if($cur!='USD'){
		if($ttype=='Buy'){
		$rem = '0';
		mysqli_query($bd,"UPDATE other_bank_account SET debit=debit+$amount, credit=credit, balance=balance+'$amount', pesoval=pesoval+'$pesoval' WHERE bank_code='$civu'");
		mysqli_query($bd,"INSERT INTO bank_transfer (trn_num, date, debit, credit, bank_name, remarks, pesoval) 
VALUES ('$tnu','$da','$amount','0','$civu','$rem','$pesoval')");
		}
		if($ttype=='Sell'){
		$rem = '0';
		mysqli_query($bd,"UPDATE other_bank_account SET debit=debit, credit=credit+$amount, balance=balance-'$amount', pesoval=pesoval-'$pesoval' WHERE bank_code='$civu'");
		mysqli_query($bd,"INSERT INTO bank_transfer (trn_num, date, debit, credit, bank_name, remarks, pesoval) 
...
$result = mysqli_query($bd,"SELECT * FROM user where id='$oper_id'");
				while($row = mysqli_fetch_array($result))
					{
					echo $row['username'];
					}
...
<table border="1" width="400" cellspacing="0" cellpadding="3" style="font-size:16px; font-family:arial;">
			...
			function formatMoney($number, $fractional=false) {
				...
				$result = mysqli_query($bd,"SELECT * FROM transaction where transaction_nu='$trans'");
				while($row = mysqli_fetch_array($result))
					{
						echo '<tr>';
					...
					echo '<tr>';
						echo '<td>';
							$result = mysqli_query($bd,"SELECT sum(amount) FROM transaction where transaction_nu='$trans'");
						...
						echo '<td>&nbsp;</td>';
						echo '<td>';
							$result = mysqli_query($bd,"SELECT sum(netconvert) FROM transaction where transaction_nu='$trans'");
							while($row2 = mysqli_fetch_array($result))
							  {
							   echo 'Total: ';
